...
FFSRCS = $(addprefix $(SRC_DIR)/, $(FSRCS) $(F90SRCS))
...
#	[ -f $(RUN_H) ] || touch $(RUN_H)
	...
	@echo '====> running CPP'
ifeq ($(MACHINE),IBM)
	for i in $(FFSRCS); do $(CPP) -M $(CPPFLAGS) -I$(SRC_DIR) $$i > `basename $$i`.cpp; \
	cat $${i%.f}.u >> $(DEPENDFILE)_cpp; done
else
ifeq ($(MACHINE),Linux)
# hack to get around use of 'traditional' in cpp call
	for i in $(FFSRCS); do $(CPP) -M $(CPPFLAGS) -I$(SRC_DIR) $$i | sed 's/\.f\.o/.o/g' >> $(DEPENDFILE)_cpp; \
	$(CPP) $(CPPFLAGS) -I$(SRC_DIR) $$i > `basename $$i`.cpp; done
else
	for i in $(FFSRCS); do $(CPP) -M $(CPPFLAGS) -I$(SRC_DIR) $$i >> $(DEPENDFILE)_cpp; \
	$(CPP) $(CPPFLAGS) -I$(SRC_DIR) $$i > `basename $$i`.cpp; done
endif
endif
	@echo '====> running fmakedep'
	$(FMAKEDEP) $(CPPFLAGS) -I$(SRC_DIR) -f $(DEPENDFILE)_f $(FFSRCS_CPP) 
	-rm -f $(FFSRCS_CPP)
