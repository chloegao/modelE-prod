...
!@var SCMin_tProfile structured type for SCM input profiles on native
!vertical grid
      type SCMin_tProfile
...
!@var SCMin_tSscalar structured type for SCM input scalars
      type SCMin_tScalar
...
!@var SCMreadXscalar structured type for reading SCM input scalars
      type SCMreadXscalar
...
!@var SCMreadZprofile structured type for reading SCM input profiles
...
!@var SCMnml_type structured type for reading SCM input variable
!namelist
      type SCMnml_type
...
C-------------------------------------------------------------------------------
      ...
      use constant, only : LHE,TF
      implicit none
      integer L
      real*8, external :: QSAT

c     report initialization
      write(6,*) 'SCM initializing ... '

c     read input data, check for required fields, do all time interpolation
      write(6,*) ' ... SCM reading inputs ...'
      call read_SCM_inputs

c     fill SCMin fields for the first time step, on the GCM vertical grid
      write(6,*) ' ... SCM initializing input variables ...'
      call update_SCM_inputs

c     initialize atmospheric state
      write(6,*) ' ... SCM initializing atmospheric state ...'
      do L = 1,LM
        T(1,1,L) = SCMin%T(L)/PK(L,1,1)
        Q(1,1,L) = SCMin%Q(L)
c       horizontal winds may be specified throughout or else geostrophic
c       throughout; if geostrophic, may be initialized using SCMopt%wind
        if( SCMopt%wind )then
          U(1,1,L) = SCMin%U(L)
          V(1,1,L) = SCMin%V(L)
...
c     initialize surface from run deck if requested
...
c     read variable namelist, which provides input file variable names
c     if they differ from model names, and any unit conversions required
c     (all input variables must be included in the namelist; 
c     included variables that are not used will be ignored)
      call read_SCM_namelist('SCM_NML')

c     specified sensible and latent heat fluxes (W/m2),
      if( SCMopt%sflx )then
        call read_SCM_scalar('SCM_SFLUX','LHF',SCMin_tLHF)
        call read_SCM_scalar('SCM_SFLUX','SHF',SCMin_tSHF)
      endif

c     specified skin temperature (K)
      if( SCMopt%Tskin )then
        call read_SCM_scalar('SCM_TSKIN','Tskin',SCMin_tTskin)
      endif

c     specified surface pressure (mb)
      if( SCMopt%Ps )then
        call read_SCM_scalar('SCM_PS','Ps',SCMin_tPs)
      endif

c     specified atmospheric temperature or potential temperature (K)
      if( SCMopt%temp )then
...
c     specified atmospheric water vapor (kg/kg)
      if( SCMopt%wvmr )then
        call read_SCM_profile('SCM_WVMR','Q',SCMin_tQ)
      endif

c     specified horizontal winds and/or geostrophic winds (m/s)
      if( SCMopt%wind )then
...
c     specified large-scale vertical wind (mb/s) or forcing terms
...
c     specified large-scale horizontal tendences of temperature (K/s)
c     and water vapor (kg/kg/s)
      if( SCMopt%ls_h )then
...
C-------------------------------------------------------------------------------
      ...
      namelist/scm_input_var/model_varname,file_varname,
     &                       scale_factor,offset
...
c     read namelist once just to check and count variables
      ...
      do
        ...
        if( trim(model_varname).eq.'notset' ) call
     &    stop_model('SCM: namelist requires model_varname',255)
        nvar_nml = nvar_nml + 1
      enddo
      call closeunit(iu_nml)

c     allocate and read all elements
      ...
      ! read values provided
        ...
        if(trim(file_varname).eq.'notset')then
          SCMreadNML(ivar)%file_varname = 
     &                           SCMreadNML(ivar)%model_varname
        else
        ...
        write(6,*) ' ... namelist variable names: ',
     &    trim(model_varname),' ',trim(file_varname)
      enddo
      call closeunit(iu_nml)
...
c     obtain file variable name, unit offset, conversion from namelist
      file_vname='notset'
      ...
      enddo
      if( file_vname=='notset' ) call
     &  stop_model('SCM: scalar not found in namelist',255)
...
c     allocate temporary storage variables
...
c     interpolate scalar time series to SCM time steps
...
c     allocate scalar size to total number of simulation time steps,
c     where the first time step is time zero by convention
      SCMin_tS%ntime = ItimeE - ItimeI + 1
      allocate(SCMin_tS%value(0:SCMin_tS%ntime-1))

c     SCM times to interpolate to, using fractional years as common unit
      ...
      do it_SCM = 2,SCMin_tS%ntime
        SCMin_time(it_SCM) = SCMin_time(it_SCM-1) + 
     &                       DTsrc/SECONDS_PER_YEAR
      enddo

c     input times to interpolate from, fractional years as common unit
      allocate(SCMread_time(SCMreadXt%ntime))
      do it_read = 1,SCMreadXt%ntime
        if( SCMreadXt%month(it_read) == 2 .and.
     &      SCMreadXt%day(it_read) > 28 ) call 
     &      stop_model('SCM: no leap years in ModelE',255)
...
c     interpolate from read times to input times, extrapolate if needed
      do it_SCM = 1,SCMin_tS%ntime
        if( SCMin_time(it_SCM) <= SCMread_time(1) )then
          SCMin_tS%value(it_SCM-1) = SCMreadXt%value(1)
        else if( SCMin_time(it_SCM) > 
     &           SCMread_time(SCMreadXt%ntime) )then
...
c     obtain file variable name, unit offset, conversion from namelist
      ...
      enddo
      if( file_vname=='notset' ) call
     &  stop_model('SCM: profile not found in namelist',255)

c     read time and height dimensions
      stat = nf_open(file_name,0,ncid)
      if(stat.ne.NF_NOERR) call handle_err(stat,file_name,'nf_open')
      stat = nf_inq_dimid(ncid,'time',dimid)
      if(stat.ne.NF_NOERR) call handle_err(stat,file_name,'id_time')
      ...
      if(stat.ne.NF_NOERR) call handle_err(stat,file_name,'time')
c     vertical grid must be pressure coordinate
...
c     read time variables and profile time series
...
c     interpolate profile time series to SCM time steps
...
c     number of levels and times for output
...
c     interpolate from read times to input times, extrapolate if needed
     ...
     &           SCMread_time(SCMreadZt%ntime) )then
          SCMin_tP%value(it_SCM-1,:) = 
     &             SCMreadZt%value(SCMreadZt%ntime,:)
        else
...
C-------------------------------------------------------------------------------
      ...
      subroutine interp_p_SCM_profile(SCMin_tP,SCMinP,model_vname)
      ! interpolate from input profile grid to GCM vertical grid
      ! (input profiles already interpolated to SCM time steps), and
      ! append climatological values of water vapor and temperature
      ! above top of input profiles but do not allow potential temperature
      ! to decrease with height (or lapse rate to exceed dry adiabat)
...
c     interpolate from pressure grid

c     continuous edge pressure grid for GCM for convenience
      Pedge(1:LM+1) = PEDN(1:LM+1,1,1)

c     loop over GCM pressure layers
      do Lgcm = 1,LM
        ...
        ! linear interpolation of vertical winds to GCM lower edges
          ...
          ! set to zero if GCM layer edge is above specified inputs
            SCMinP(Lgcm) = 0.
          else
...
c         identify layer range of input data within the GCM layer
          Lbot = 0
          Ltop = 0
...
c         pressure-weighted conversion to GCM layers with inputs,
c         variable-dependent treatment in GCM layers above
          ...
          ! (neglect contribution from input data)
            if( model_vname=='U' ) SCMinP(Lgcm) = SCMinP(Lgcm-1)
            if( model_vname=='V' ) SCMinP(Lgcm) = SCMinP(Lgcm-1)
            if( model_vname=='Q' )then
              call append_SCM_profile(PMID(Lgcm,1,1),SCMinP(Lgcm),'Q')
            endif
            if( model_vname=='T' )then
              call append_SCM_profile(PMID(Lgcm,1,1),SCMinP(Lgcm),'T')
              ! potential temperature cannot decrease with height
              SCMinP(Lgcm) = PK(Lgcm,1,1)*
     &            max(SCMinP(Lgcm)  /PK(Lgcm,1,1),
     &                SCMinP(Lgcm-1)/PK(Lgcm-1,1,1))
            endif
            if( model_vname=='TH' )then
              call append_SCM_profile(PMID(Lgcm,1,1),SCMinP(Lgcm),'TH')
              ! potential temperature cannot decrease with height
...
c     find closest match from season and latitude options
      ...
      if( model_vname .eq. 'Q' ) X_value = Q
      if( model_vname .eq. 'T' ) X_value = T
      if( model_vname .eq. 'TH' ) X_value = T/(P_layer/1000.)**KAPA
...
c     specified surface fluxes
      if( SCMopt%sflx )then
        SCMin%lhf = SCMin_tLHF%value(nstepSCM)
        SCMin%shf = SCMin_tSHF%value(nstepSCM)
      endif

c     specified skin temperature
      if( SCMopt%Tskin ) SCMin%Tskin = SCMin_tTskin%value(nstepSCM)

c     specified temperature or potential temperature with 1000-mb ref
      if( SCMopt%temp )then
        call interp_p_SCM_profile(SCMin_tT,SCMin%T,'T')
      endif
      if( SCMopt%theta )then
        call interp_p_SCM_profile(SCMin_tTH,SCMin%TH,'TH')
        do L = 1,LM
          SCMin%T(L) = SCMin%TH(L)*(PMID(L,1,1)/1000.)**KAPA
        enddo
      endif

c     specified water vapor mixing ratio
      if( SCMopt%wvmr )then
        call interp_p_SCM_profile(SCMin_tQ,SCMin%Q,'Q')
      endif

c     specified winds: if horizontal winds and geostrophic winds
c     are both specified, former will be used only for initialization
      if( SCMopt%wind )then
        call interp_p_SCM_profile(SCMin_tU,SCMin%U,'U')
        call interp_p_SCM_profile(SCMin_tV,SCMin%V,'V')
      endif
      if( SCMopt%geo )then
        call interp_p_SCM_profile(SCMin_tUg,SCMin%Ug,'Ug')
        call interp_p_SCM_profile(SCMin_tVg,SCMin%Vg,'Vg')
      endif

c     large-scale forcing terms
c     vertical velocity always applied through omega, in pressure units (mb/s)
      if( SCMopt%omega )then
        call interp_p_SCM_profile(SCMin_tOmega,SCMin%Omega,'Omega')
      endif
      if( SCMopt%w )then
        call interp_p_SCM_profile(SCMin_tW,SCMin%W,'W')
        do L = 1,LM
          SCMin%Omega(L) = -SCMin%W(L)*GRAV*PMID(L,1,1)/
     &                      (RGAS*T(1,1,L)*PK(L,1,1))
        enddo
      endif
      if( SCMopt%ls_v )then
        call interp_p_SCM_profile(SCMin_tSadvV,SCMin%SadvV,
     &                           'SadvV')
        call interp_p_SCM_profile(SCMin_tQadvV,SCMin%QadvV,
     &                           'QadvV')
      endif
      if( SCMopt%ls_h )then
        call interp_p_SCM_profile(SCMin_tTadvH,SCMin%TadvH,
     &                           'T_adv_h')
        call interp_p_SCM_profile(SCMin_tQadvH,SCMin%QadvH,
     &                           'QadvH')
      endif
