...
!@var N_XXX: variable names of indices for tracers (init = 0)
     ...
     *     n_Terpenes=0,n_codirect=0, n_Acetone=0,
...
!        needed for exact restarts:
     ...
     &     variable_exists
      ...
      public :: deftrvar,io_trvar

      interface deftrvar
      module procedure deftrvar_2d
      module procedure deftrvar_3d
      module procedure deftrvar_4d
      end interface

      interface io_trvar
      module procedure io_trvar_2d
      module procedure io_trvar_3d
      module procedure io_trvar_4d
      end interface

      contains

      subroutine setup_3d(jdim,trdim,jdim_,d3)
      integer :: jdim,trdim,jdim_,d3
!
      integer :: td
      if(trdim < 1 .or. trdim > 4) then
        call stop_model('bad trdim',255)
      endif
      if(trdim < jdim) then
        jdim_ = jdim - 1
      else
      ...
      td = mod(5 + trdim - jdim, 4) + 1 ! rotate to jdim=2
      if(td < 3) call stop_model('bad trdim or jdim',255)
      d3 = mod(4 - td + jdim, 4) + 1
      end subroutine setup_3d

      subroutine setup_4d(jdim,trdim,jdim_,d3,d4)
      integer :: jdim,trdim,jdim_,d3,d4
!
      integer :: td
      if(trdim < 1 .or. trdim > 5) then
        call stop_model('bad trdim',255)
      endif
      if(trdim < jdim) then
        jdim_ = jdim - 1
      ...
      td = mod(6 + trdim - jdim, 5) + 1 ! rotate to jdim=2
      if(td < 3) call stop_model('bad trdim or jdim',255)
      ...
      subroutine deftrvar_2d(fid,namestr,dimstr,trdim,trvar,
     &     filter)
      character(len=*) :: namestr,dimstr
      integer :: fid,trdim
      logical, dimension(ntm), intent(in) :: filter
      real*8, dimension(:,:,:) :: trvar
!
      integer :: il,iu,jl,ju,n,jdim
      real*8, dimension(:,:), allocatable :: tr1
      character(len=128) :: varstr

      if(size(trvar,trdim).ne.ntm) then
        call stop_model('ntm dimsize mismatch',255)
      endif
      if(trdim.ne.1 .and. trdim.ne.3) then
        call stop_model('bad trdim',255)
      endif
      ...
      il = lbound(trvar,jdim-1)
      iu = ubound(trvar,jdim-1)
      jl = lbound(trvar,jdim)
      ju = ubound(trvar,jdim)

      allocate(tr1(il:iu,jl:ju))

      do n=1,ntm
        if(.not.filter(n)) cycle
        varstr = trim(namestr)//'_'//trim(trname(n))//trim(dimstr)
        call defvar(grid,fid,tr1,varstr)
      enddo

      end subroutine deftrvar_2d

      subroutine io_trvar_2d(fid,actstr,namestr,trdim,trvar,filter)
      character(len=*) :: actstr,namestr
      integer :: fid,trdim
      logical, dimension(ntm), intent(in) :: filter
      real*8, dimension(:,:,:) :: trvar
!
      ...
      do n=1,ntm
        if(.not.filter(n)) cycle
        vname = trim(namestr)//'_'//trim(trname(n))
      ...
      end subroutine io_trvar_2d

      subroutine deftrvar_3d(fid,namestr,dimstr,jdim,trdim,trvar,
     &     filter)
      character(len=*) :: namestr,dimstr
      integer :: fid,jdim,trdim
      logical, dimension(ntm), intent(in) :: filter
      real*8, dimension(:,:,:,:) :: trvar
!
      integer :: il,iu,jl,ju,d3,jdim_,n
      real*8, dimension(:,:,:), allocatable :: tr1
      character(len=128) :: varstr


      call setup_3d(jdim,trdim,jdim_,d3)
      if(size(trvar,trdim).ne.ntm) then
        call stop_model('ntm dimsize mismatch',255)
      endif
      d3 = size(trvar,d3)
      ...
      end subroutine deftrvar_3d

      subroutine io_trvar_3d(fid,actstr,namestr,jdim,trdim,trvar,filter)
      character(len=*) :: actstr,namestr
      integer :: fid,jdim,trdim
      logical, dimension(ntm), intent(in) :: filter
      real*8, dimension(:,:,:,:) :: trvar
!
      integer :: il,iu,jl,ju,d3,d3l,d3u,jdim_,n
      real*8, dimension(:,:,:), allocatable :: tr1
      character(len=128) :: vname
      ...
      end subroutine io_trvar_3d

      subroutine deftrvar_4d(fid,namestr,dimstr,jdim,trdim,trvar,
     &     filter)
      character(len=*) :: namestr,dimstr
      integer :: fid,jdim,trdim
      logical, dimension(ntm), intent(in) :: filter
      real*8, dimension(:,:,:,:,:) :: trvar
!
      integer :: il,iu,jl,ju,d3,d4,jdim_,n
      real*8, dimension(:,:,:,:), allocatable :: tr1
      character(len=128) :: varstr

      call setup_4d(jdim,trdim,jdim_,d3,d4)
      ...
      end subroutine deftrvar_4d

      subroutine io_trvar_4d(fid,actstr,namestr,jdim,trdim,trvar,filter)
      character(len=*) :: actstr,namestr
      integer :: fid,jdim,trdim
      logical, dimension(ntm), intent(in) :: filter
      real*8, dimension(:,:,:,:,:) :: trvar
!
      integer :: il,iu,jl,ju,d3,d4,d3l,d3u,d4l,d4u,jdim_,n
      real*8, dimension(:,:,:,:), allocatable :: tr1
      character(len=128) :: vname
