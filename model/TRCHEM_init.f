...
!@auth Kostas Tsigaridis
     ...
     &                 aerosols_affect_photolysis)
      ...
      ! function of pressure in masterchem routine:
      if(plbot(min(jpnl,topLevelOfChemistry)+1) >= 1.d0) call
     & stop_model('jpnl or topLevelOfChemistry should be higher',255)
      ncfastj2=2*NLGCM+2
      nbfastj=NLGCM+1
      j_iprn=iprn
      j_jprn=jprn
